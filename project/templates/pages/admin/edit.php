...
?>
    <form method="post" action="/admin/update/<?= htmlspecialchars($car->id) ?>">
        <div class="mt-8 max-w-md">
            <div class="grid grid-cols-1 gap-6">
                <div class="block">
                    <label for="field1" class="text-gray-700 font-bold">Введите название модели</label>
                    <input required id="field1" value="<?= htmlspecialchars($car->name) ?>" name="name" type="text"
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                           placeholder="KIA RIO">
                </div>
                <div class="block">
                    <label for="field2" class="text-gray-700 font-bold">Введите цену с скидкой</label>
                    <input required id="field2" value="<?= htmlspecialchars($car->price) ?>" name="price" type="number"
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                           placeholder="1000000">
                </div>
                <div class="block">
                    <label for="field3" class="text-gray-700 font-bold">Введите цену без скидки</label>
                    <input id="field3" value="<?= htmlspecialchars($car->old_price) ?>" name="old_price" type="number"
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                           placeholder="1500000">
                </div>
                <div class="block">
                ...
                <div class="block">
                    <label for="field5" class="text-gray-700 font-bold">Отображать на главной странице?</label>
                    <select id="field5" name="is_published"
                            class="block w-full mt-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                        <option <?= $car->is_published == 1 ? 'selected' : '' ?> value="1">Да</option>
                        <option <?= $car->is_published == 0 ? 'selected' : '' ?> value="0">Нет</option>
                    </select>
                </div>
                <div class="block">
                    <div class="mt-2">
                        <div>
                            <?php foreach ($colors as $color) { ?>
                                <label class="inline-flex items-center cursor-pointer">
                                    <input <?= in_array($color->id, $array) ? 'checked' : '' ?> type="checkbox"
                                                                                                name="colors[]"
